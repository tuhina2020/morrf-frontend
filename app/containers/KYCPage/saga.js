...
function* uploadPanImage({ payload }) {
  ...
  try {
    ...
    yield put(setPanImage({ id: '', images: [], done: false }));
  }
}
function* uploadSignatureImage({ payload }) {
  ...
  try {
    ...
    yield put(setSignatureImage({ id: '', images: [], done: false }));
  }
}
function* setRemoteSignatureDetails({ payload }) {
  ...
  try {
    ...
    debugger;
    setToast({
      message: 'Successfully addeed Signature',
      mode: 'info',
    });
...
function* editSignatureDetails({ payload }) {
  yield put(setLoading(true));
  const kycPage = yield select(makeSelectKYCPage());
  const loginData = getDefaultState('loginData', {});
  const user_id = loginData.id;
  ...
  try {
    ...
    debugger;
    setToast({
      message: 'Successfully Edited Signature',
      mode: 'info',
    });
    yield put(getUserPan());
  } catch (err) {
...
function* removePanImage({ payload }) {
  const { index, id } = payload;
  const kycPage = yield select(makeSelectKYCPage());
  const {
    panImage: { images },
...
function* removeSignatureImage({ payload }) {
  const { index, id } = payload;
  const kycPage = yield select(makeSelectKYCPage());
  const {
    signatureImage: { images },
...
export default function* KYCPageSaga() {
  ...
  yield takeLatest(UPLOAD_SIGNATURE_IMAGE, uploadSignatureImage);
  ...
  yield takeLatest(REMOVE_PAN_IMAGE, removePanImage);
  yield takeLatest(REMOVE_SIGNATURE_IMAGE, removeSignatureImage);
  ...
  yield takeLatest(SET_REMOTE_SIGNATURE, setRemoteSignatureDetails);
  yield takeLatest(EDIT_SIGNATURE, editSignatureDetails);
}
